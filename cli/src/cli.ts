...
import { setRegistry, type Extension } from './registry'
...
/**
 * Build and run the CLI against a caller-supplied extension registry.
 *
 * The registry is injected rather than imported so the CLI stays decoupled from
 * any single repo: this repo passes its own `extensions`, a standalone extension
 * repo passes `[MyExtension]`. See README.md § Using it in your own repo.
 */
export const runCli = (
  registry: Extension[],
    ...
    .command(
      ...
      (y) =>
        y
          ...
          .option('fields', {
            type: 'string',
            describe: 'Action fields as JSON',
          })
          .option('fields-file', {
            type: 'string',
    ...
    .command(
      ...
      async (argv) => {
        ...
        await replayWebhook({
          ...
          payload,
          headers:
            Object.keys(headers).length > 0 ? (headers as any) : undefined,
          json: argv.json,
        })
      },
    )
