...
import { MODEL_VERSIONS, OPENAI_MODELS, getDefaultConfig } from './constants'
...
export const createOpenAIModel = async ({
...
}): Promise<OpenAIModelConfig> => {
  const apiKey = settings.openAiApiKey ?? helpers.getOpenAIConfig().apiKey

  if (isNil(apiKey)) {
    throw new Error(
      'No OpenAI API key available in settings or environment configuration',
    )
  }
  ...
  if (hideDataForTracing) {
    ...
    const maskOutputs = (
      ...
      const maskedGenerations = outputs.generations.map(
        (generationArray: any[]) =>
            ...
            message: {
              ...
              kwargs: {
                ...
                response_metadata: {
                  ...
                  finish_reason:
                    generation.message?.kwargs?.response_metadata
                      ?.finish_reason,
                },
              },
            },
