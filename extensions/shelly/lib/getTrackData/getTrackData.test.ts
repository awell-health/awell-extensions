...
describe('getTrackData', () => {
  ...
  test('Should return processed track data with steps and activities', async () => {
    ...
    const firstCallArgs = (awellSdkMock.orchestration.query as jest.Mock).mock
      .calls[0][0]
    expect(firstCallArgs).toHaveProperty(
      'careflowActivities.__args.pathway_id',
      'test-pathway-id',
    )
    expect(firstCallArgs).toHaveProperty(
      'careflowActivities.__args.filters.track_id',
      'test-track-id',
    )
    expect(firstCallArgs).toHaveProperty(
      'pathwayDataPoints.__args.pathway_id',
      'test-pathway-id',
    )
    ...
    expect(result.steps[0].activities[2].form).toHaveProperty(
      'title',
      'Test Form',
    )
    ...
    expect(result.steps[1]).toMatchObject({
      ...
      label: 'Step 2',
      status: 'SCHEDULED',
      start_date: '2023-01-03T10:00:00.000Z',
      end_date: null,
    })
  ...
  test('Should fetch and process message content successfully', async () => {
    ...
    const messageActivity = step1?.activities.find(
      (activity) =>
        activity.subject.type === 'AWELL' && activity.object.type === 'MESSAGE',
    )
