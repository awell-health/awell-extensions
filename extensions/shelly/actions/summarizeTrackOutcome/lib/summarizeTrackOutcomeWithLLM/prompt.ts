...
- **Action/Activity:** Individual events within a step, such as forms, messages, or automated system events (e.g., calculations, API calls).  
  *Note:* When an action like a message or form is assigned to a stakeholder, interpret it as **"delivered/sent"** in standard language.
...
- **Discarded Steps & Activities:**  
  ...
  - **Never mention discarded steps in the final summary unless explicitly instructed. This is critical. So avoid mentioning things like: The alternative treatment proposal (ATP) was not pursued, as indicated by the discarded "Hernia Post-Op NOT Supported" phase.**  
  - This is **critical**—your summary must only focus on what actually happened.
- **Data Integrity:**  
  - Base your analysis **solely** on the provided data.  
  - Absolutely refrain from inventing details or making clinical assumptions beyond what is explicitly given. This is critical for maintaining trust.
...
- **Only describe completed actions**—do not include planned future steps or what happens next.  
  - **Correct:**  
    **Outcome:** The alternate treatment plan (ATP) was not accepted.  
  - **Incorrect:**  
    **Outcome:** The alternate treatment plan (ATP) was not accepted, and the case was directed toward pre-certification.  
...
- **Accuracy:**  
  ...
  - Absolutely refrain from making assumptions or clinical conclusions that are not directly supported by the input. For example, refrain from adding comments like "which are critical factors in determining the need for surgical intervention" unless that is explicitly stated in the provided data.
- **Conciseness:**  
  - Your final summary should be **succinct yet informative**, with a logical, story-like progression.
...
## **Details Supporting the Outcome:**
- During the initial hernia triage, it was determined that the hernia was reducible and did not affect the patient's activities of daily living (ADLs).
