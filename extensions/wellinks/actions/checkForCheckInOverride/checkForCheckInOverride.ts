import {
  ...
  function parseListOfForms(data: GetChartingItemsQuery["chartingItems"], eventTypeQuestionId: string, startDateQuestionId: string, appointmentTime: string): boolean {
      if(isNil(data) || isNil(appointmentTime) || isNil(eventTypeQuestionId) || isNil(startDateQuestionId)) {
        return false
      } else {
          const overrideForms = data.filter(
              (value) => value?.form_answer_group?.form_answers.find((value) => value.custom_module_id === eventTypeQuestionId)?.answer === "Override check-in automation"
          )
