...
export const checkForScheduledAppointment: Action< typeof fields, typeof settings, keyof typeof dataPoints> = {
    ...
    onActivityCreated: async (payload, onComplete, onError): Promise<void> => {
        ...
        try {
            ...
            if (!isNil(client)) {
                ...
                const { data } = await sdk.getScheduledAppointments({user_id: patientId, appointment_type_id: appointmentTypeId, status: "" })
                        ...
                        events: [
                            {
                                ...
                                text: { en: 'There was an error in getting Appointments from Healthie' },
                                error: {
                ...
                await onError({
                    events: [
                        {
                            date: new Date().toISOString(),
                            text: { en: 'There was in error in setting up the GraphQL Client' },
                            error: {
                                category: 'SERVER_ERROR',
