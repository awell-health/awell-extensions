...
| 385221394, 385221395 — group **11941507** (sast, medium 45) | `cli/src/index.ts` — `readJsonFile()` and the `--payload-file` branch of `webhook replay` | Nothing to fix: `awell-ext` is a local developer CLI and every path it reads (`--fields-file`, `--headers-file`, `--payload-file`) is the operator's own argument on their own machine — the same call as `cat`. Suppressed with `// nosemgrep: AIK_ts_generic_path_traversal` on both `readFileSync` lines (2026-09-08); the CLI is not shipped in the extension-server image. | `AIK_ts_generic_path_traversal` is a sink detector keyed on `readFileSync(<argv-derived>)`; no guard changes that. Re-open if the CLI ever runs server-side or on a path a request supplies. |
| 16462374 (leaked_secret, medium 60) | `extensions/awell/v1/actions/startHostedPagesSession/startHostedPagesSession.ts:38` | **History only.** An `apiKey` literal was committed in 045ee746 and removed the same day in f693e53e (2024-11-06); the line the feed points at now reads `await delay(4000)`. Nothing in the tree to change. Whether the key was ever live in a real tenant, and rotating it, is the owner's call — the finding is closed from the dashboard by a human once that is settled. | Aikido flags the commit, not the tree. |
| 180286530, 180286531 (leaked_secret, medium 60) | `extensions/icd/client.ts:66`, `extensions/icd/actions/tfu/icd.ts:116` | **History only.** The whole `extensions/icd` tree was removed in #434 (ff448e64). Same handling as the row above. | as above |
